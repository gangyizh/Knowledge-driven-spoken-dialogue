...
class IntentInfere():
    def __init__(self, gpu, pretrain_model_path, save_model_path, max_seq_len, max_attrname_len, max_attrvalue_len):
    ...
    def text_smiliary(self, text1, text2, text3, bacth_size = 64):
        '''
        text1,text2,text3: list(str)
        '''

        text1 = [truncate_str(t, self.max_seq_len) for t in text1]
        text2 = [truncate_str(t, self.max_attrname_len) for t in text2]
        text3 = [truncate_str(t, self.max_attrvalue_len) for t in text3]

        text4 = [t2+self.tokenizer.sep_token+t3 for t2,t3 in zip(text2, text3)]
