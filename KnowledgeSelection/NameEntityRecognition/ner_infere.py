...
class NERInfere():
    ...
    def ner(self, text):
        if(text in self.cache_mp):
            return self.cache_mp[text]
        ...
        books = re.findall('《(.*?)》', text)                      ### 规则: 书名号
        for x in books:
            if x not in entities:
                entities.append(x)
        en_text = re.findall(r'[A-Za-z ]+',text)

        entities = [entity for entity in entities if len(entity)>0]

        if len(en_text)>0:
            english_ent = en_text[0]
            entities = [entity if entity not in english_ent else english_ent for entity in entities]
        self.cache_mp[text] = entities
        return entities
