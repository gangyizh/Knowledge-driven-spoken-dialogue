...
def get_ch_token(s):
    ...
    tokens = list(''.join(re.findall(zh_char,s)))+re.findall(en_letter,s)+re.findall(digit_letter, s)
    return tokens
...
class UnigramMetric(Metric):
    ...
    def update(self, output):
        ...
        hyp_tokens = get_ch_token(hypothesis)
        ref_tokens = get_ch_token(reference)
