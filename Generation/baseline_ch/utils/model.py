...
def run_batch_generation(args, model, batch):
    ...
    input_ids, attention_mask, token_type_ids, lm_labels = batch
    model_outputs = model(input_ids=input_ids,  attention_mask=attention_mask, labels=lm_labels) # token_type_ids=None,
    ...
    for i in range(args.max_length):
        ...
        if i < args.min_length and prev.item() in special_tokens_ids:
            while prev.item() in special_tokens_ids:
                if probs.max().item() == 1:           ### 可以修改threshold
...
def run_batch_generation_seq2seq_sample(args, model, batch, dataset):
    special_tokens_ids = args.tokenizer.convert_tokens_to_ids(dataset.SPECIAL_TOKENS_VALUES)
    current_output = []
    ...
    input_ids = torch.tensor(instance["input_ids"], device=args.device).unsqueeze(0)
    current_output = model.generate(input_ids, do_sample=False,  max_length=200, num_beams=1, eos_token_id=args.tokenizer.convert_tokens_to_ids("<eos>"))
